    ...
    with open(inputfile, 'r') as f: #opens whichever file is specified
        ...
        for line in f:
            ...
            if line.startswith('#CHROM'): #ignores all the header lines
                ...
                genoPLUSmantatypelist=[] #empty list to which genotype depths will be added later
                #print(len(genotypes))
...
#                    totaldepth_atlocus = genos[2] #totaldepth at locus
        ...
        #
                    ...
                    genoPLUSmantatype = genostring + "," + mantatype#gives a string of the genotype plus its corresponding depthsand GQscore

                    genoPLUSmantatypelist.append(genoPLUSmantatype)
                    genoPLUSmantatype = '\t'.join(genoPLUSmantatypelist)
                ...
                if geno_list[rep1] == geno_list[rep2]: #outputs just the sites where the two replicate libraries are the same genotype
                    ...
                    outlist = [concatenated, ref_allele, alt_allele, genostring, genoPLUSmantatype] # collect each of these for each line where the genotypes are the same
                    outstring = '\t'.join(outlist)
                    #print(outlist)
                    #print(genoPLUSmantatype)
